...
static void fan54015_vbus_det_work(struct work_struct *work)
{
	...
	if (vbus_gpio) {
		...
		if ((!vbus_gpio_value) && (otg_enable_flag == 1)) {
			...
			fan54015_set_value(FAN5405_REG_OREG,
				FAN5405_OTG_EN, FAN5405_OTG_EN_SHIFT, 1);
		}
...
void fan54015_init(void)
{
	...
	if (vendor_id == VENDOR_FSC5425) {
		SPRD_EX_DEBUG("fsc5425_init\n");
		fan54015_sw_reset();
		/* Reg 1 INLIM = 500mA bit7,bit6 */
		fan54015_set_value(FAN5405_REG_CONTROL1, FAN5405_IINLIM,
			FAN5405_IINLIM_SHIFT, NOLIMIT);

		/* Reg 5 IO_LEVEL is 0.Output current
		 * is controlled by IOCHARGE bits.
		 */
		fan54015_set_value(FAN5405_REG_SP_CHARGER, FAN5405_VSP,
			FAN5405_VSP_SHIFT, VSP4P533);
		for (i = 0; i < 11; i++) {
			fan54015_read_reg(i, &data);
...
void fan54015_otg_enable(int enable)
{
	...
	if (enable) {
		...
		fan54015_set_value(FAN5405_REG_OREG,
                                FAN5405_OTG_EN, FAN5405_OTG_EN_SHIFT, enable);
...
void fan54015_stop_charging(void)
{
	...
	if (en_gpio) {
		...
		if (vendor_id == VENDOR_FSC5425) {
			...
			SPRD_EX_DEBUG("stop set 1 en_gpio gpio:%d\n", en_gpio);
		}
	}
}

void fan54015_stop_status(void)
{
	unsigned char vendor_id;
...
{
	return fan54015_get_value(FAN5405_REG_IC_INFO, (0x3 << 3), 3);
}
...
void fan54015_enable_chg(void)
{
	...
	if (en_gpio) {
		...
		if (vendor_id == VENDOR_FSC5425) {
			gpio_direction_output(en_gpio, 0);
			SPRD_EX_DEBUG("enable set 0 en_gpio:%d\n", en_gpio);
		}
	}
}
			  ...
			  const struct i2c_device_id *id)
{
	...
	temp = of_get_named_gpio(np, "chg-en-gpios", 0);
	if (gpio_is_valid(temp)) {
		en_gpio = temp;
		SPRD_EX_DEBUG("chg en_gpio =%d\n", en_gpio);
		rc = gpio_request(en_gpio, "charge en gpio");
		if (rc) {
			SPRD_EX_DEBUG("already request en_gpio gpio:%d\n",
				en_gpio);
		}
		gpio_direction_output(en_gpio, 0);
	} else {
