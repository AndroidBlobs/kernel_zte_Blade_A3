...
unsigned char sprdchg_fsc5425_cur2reg(uint32_t cur)
{
	unsigned char reg_val;
	...
	if ((cur >= 650) && (cur < 1200))
		reg_val = 1;
	if ((cur >= 1200) && (cur < 1500))
		reg_val = 2;
	if ((cur >= 1500) && (cur < 1800))
		reg_val = 3;
	if ((cur >= 1800) && (cur < 2000))
		reg_val = 4;
	if ((cur >= 2000) && (cur < 2300))
		reg_val = 5;
	if ((cur >= 2300) && (cur < 2410))
		reg_val = 6;
	if (cur >= 2410)
		reg_val = 7;
	return reg_val;
...
unsigned char sprdchg_fsc5415_cur2reg(uint32_t cur)
{
	unsigned char reg_val;
...
unsigned int sprdchg_fsc5425_reg2cur(unsigned char reg_val)
{
	unsigned int cur = 0;
...
unsigned int sprdchg_fsc5415_reg2cur(unsigned char reg_val)
{
	unsigned int cur = 0;
...
void sprdchg_fan54015_stop_status(void)
{
	fan54015_stop_status();
}
...
void sprdchg_fan54015_set_cur(uint32_t cur)
{
	...
	if (vendor_id == VENDOR_FSC5425) {
		vendor_id2 = fan54015_get_vendor_id2();
		if (vendor_id2 == 0x01)
			reg_val = sprdchg_fsc5425_cur2reg(cur);
		if (vendor_id2 == 0x02)
			reg_val = sprdchg_fsc5415_cur2reg(cur);
	}
...
unsigned int sprdchg_fan54015_get_chgcur(void)
{
	...
	if (vendor_id == VENDOR_FSC5425) {
		vendor_id2 = fan54015_get_vendor_id2();
		if (vendor_id2 == 0x01)
			cur = sprdchg_fsc5425_reg2cur(reg_val);
		if (vendor_id2 == 0x02)
			cur = sprdchg_fsc5415_reg2cur(reg_val);
	}
	SPRDCHG_DEBUG("%s cur =%d\n", __func__, cur);
	return cur;
}
...
int sprdchg_fan54015_get_chgcur_step(int cur, int increase)
{
	...
	if (vendor_id == VENDOR_FSC5425) {
		if (vendor_id2 == 0x01)
			new_cur = sprdchg_fsc5425_reg2cur(reg_val);
		if (vendor_id2 == 0x02)
			new_cur = sprdchg_fsc5415_reg2cur(reg_val);
	}
...
void sprdchg_fan54015_init(struct sprdbat_drivier_data *bdata)
{
	...
	fan54015_init();
	if (vendor_id != VENDOR_FSC5425)
		sprdchg_fan54015_set_vindpm
			(bat_drv_data->pdata->chg_end_vol_pure);
	...
	if (vendor_id == VENDOR_FSC5425) {
		sprdchg_fan54015_set_safety_vol
			(bat_drv_data->pdata->chg_end_vol_pure);
		sprdchg_fan54015_set_safety_cur
			(bat_drv_data->pdata->adp_dcp_cur);
	}
}
...
struct sprd_ext_ic_operations sprd_54015_op = {
	...
	.charge_stop_status = sprdchg_fan54015_stop_status,
};
