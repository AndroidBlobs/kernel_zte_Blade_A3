...
static struct device_attribute sprd_caliberate[] = {
	...
	SPRDBAT_CALIBERATE_ATTR(bat_id),
};
...
static int sprdbat_stop_status(void)
{
	if (sprd_ext_ic_op->charge_stop_status)
		sprd_ext_ic_op->charge_stop_status();
	SPRDBAT_DEBUG("sprdbat_stop_stauts\n");
	return 0;
}
...
static inline void _sprdbat_clear_stopflags(uint32_t flag_msk)
{
	...
	if (sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_END_NONE_BIT) {
		...
		SPRDBAT_CHG_END_TEMP_VOL_BIT)
		sprdbat_data->bat_info.bat_health =
			POWER_SUPPLY_HEALTH_TEMP_VOL;
	else if (sprdbat_data->bat_info.chg_stop_flags &
...
static void sprdbat_change_module_state(uint32_t event)
{
	...
	case SPRDBAT_CHG_TEMP_VOL_E:
		...
		_sprdbat_set_stopflags((uint32_t)
			SPRDBAT_CHG_END_TEMP_VOL_BIT);
		break;
	case SPRDBAT_TEMP_VOL_RECHARGE_E:
		_sprdbat_clear_stopflags((uint32_t)
			SPRDBAT_CHG_END_TEMP_VOL_BIT);
		break;
	default:
...
static int plugout_callback(void)
{
	...
	sprdbat_data->fchg_det = 0;
	if (sprdbat_data->stop_status)
		sprdbat_data->stop_status();
	mutex_unlock(&sprdbat_data->lock);
...
static void sprdbat_chg_rechg_monitor_z(void)
{
	SPRDBAT_DEBUG("sprdbat_chg_rechg_monitor_z\n");

	if ((sprdbat_data->bat_info.vbat_ocv <=
		sprdbat_data->pdata->rechg_vol_z) ||
		sprdbat_data->bat_info.cur_temp <= 420) {
		SPRDBAT_DEBUG("yes, chg restart_z\n");
		sprdbat_change_module_state(SPRDBAT_TEMP_VOL_RECHARGE_E);
		sprdbat_data->start_charge();
	} else {
		SPRDBAT_DEBUG("no,don't need recharge z\n");
	}
}
...
static void sprdbat_chg_status_monitor_z(void)
{
	if ((sprdbat_data->bat_info.vbat_vol >
		sprdbat_data->pdata->chg_end_vol_z)
		&& (sprdbat_data->bat_info.bat_current <
		sprdbat_data->pdata->chg_end_cur_z)
		&& sprdbat_data->bat_info.cur_temp >= 450) {
		sprdbat_data->chg_trigger_cnt_z++;
		if (sprdbat_data->chg_trigger_cnt_z >= 2) {
			SPRDBAT_DEBUG("charge vol and temp stop charge\n");
			sprdbat_data->chg_trigger_cnt_z = 0;
			sprdbat_change_module_state
				(SPRDBAT_CHG_TEMP_VOL_E);
			sprdbat_data->stop_charge();
		}
	} else {
		sprdbat_data->chg_trigger_cnt_z = 0;
...
static void sprdbat_charge_works(struct work_struct *work)
{
	...
	if (sprdbat_data->bat_info.chg_stop_flags
				& SPRDBAT_CHG_END_TEMP_VOL_BIT)
		sprdbat_chg_rechg_monitor_z();

	sprdbat_chg_status_monitor_z();
...
static void print_pdata(struct sprd_battery_platform_data *pdata)
{
	...
	SPRDBAT_DEBUG("chg_end_vol_z:%d\n", pdata->chg_end_vol_z);
	SPRDBAT_DEBUG("chg_end_cur_z:%d\n", pdata->chg_end_cur_z);
	SPRDBAT_DEBUG("rechg_vol_z:%d\n", pdata->rechg_vol_z);
							   ...
							   *pdev)
{
	...
	temp = of_get_named_gpio(np, "sprd,prechg-led-gpios", 0);
	if (gpio_is_valid(temp)) {
		pdata->gpio_prechg_led = (uint32_t) temp;
		SPRDBAT_DEBUG("gpio_prechg_led support=%d\n",
				pdata->gpio_prechg_led);
		ret = devm_gpio_request_one(&pdev->dev, pdata->gpio_prechg_led,
				GPIOF_OUT_INIT_LOW, "prechg_led_gpio");
		if (ret) {
			dev_err(&pdev->dev, "gpio%d request failed, ret %d\n",
				pdata->gpio_prechg_led, ret);
		}
	} else {
		pdata->gpio_prechg_led = 0;
		SPRDBAT_DEBUG("gpio_prechg_led do Not support =%d\n",
				pdata->gpio_prechg_led);
	}
	...
	if (of_property_read_u32(np, "chg-end-vol-z", &pdata->chg_end_vol_z))
		dev_err(&pdev->dev, "parse chg-end-vol-z err\n");

	if (of_property_read_u32(np, "chg-end-cur-z", &pdata->chg_end_cur_z))
		dev_err(&pdev->dev, "parsechg-end-cur-z err\n");

	if (of_property_read_u32(np, "rechg-vol-z", &pdata->rechg_vol_z))
		dev_err(&pdev->dev, "parse rechg-vol-z err\n");
...
static int sprdbat_probe(struct platform_device *pdev)
{
	...
	data->stop_status = sprdbat_stop_status;
