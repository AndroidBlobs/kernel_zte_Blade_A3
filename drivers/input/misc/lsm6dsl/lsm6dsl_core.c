...
static const struct lsm6dsl_sensor_name {
	...
	[LSM6DSL_ACCEL] = {
		.name = "accel",
		.description = "accelerometer", // "ST LSM6DSL Accelerometer Sensor",
	},
	[LSM6DSL_GYRO] = {
		.name = "gyro",
		.description = "gyroscope", // "ST LSM6DSL Gyroscope Sensor",
	},
#ifdef EMBEDDED_FUNCTION_SIGN_MOTION_SUPPORT
	[LSM6DSL_SIGN_MOTION] = {
		.name = "sign_m",
		.description = "ST LSM6DSL Significant Motion Sensor",
	},
#endif
#ifdef EMBEDDED_FUNCTION_STEP_COUNTER_SUPPORT
	[LSM6DSL_STEP_COUNTER] = {
		.name = "step_c",
		.description = "ST LSM6DSL Step Counter Sensor",
	},
#endif
#ifdef EMBEDDED_FUNCTION_STEP_DETECTOR_SUPPORT
	[LSM6DSL_STEP_DETECTOR] = {
		.name = "step_d",
		.description = "ST LSM6DSL Step Detector Sensor",
	},
#endif
#ifdef EMBEDDED_FUNCTION_TILT_SUPPORT
	[LSM6DSL_TILT] = {
		.name = "tilt",
...
#if (CONFIG_ST_HARDWARE_INTERRUPT)
...
#if defined(EMBEDDED_FUNCTION_SIGN_MOTION_SUPPORT)||defined(EMBEDDED_FUNCTION_STEP_COUNTER_SUPPORT)||defined(EMBEDDED_FUNCTION_STEP_DETECTOR_SUPPORT)||defined(EMBEDDED_FUNCTION_TILT_SUPPORT)
...
static int lsm6dsl_allocate_workqueue(struct lsm6dsl_data *cdata)
{
	...
	if (!lsm6dsl_workqueue)
		...
		lsm6dsl_workqueue = alloc_workqueue("%s", WQ_HIGHPRI | WQ_UNBOUND |
		 ...
		 #else
		 lsm6dsl_workqueue = create_freezable_workqueue("lsm6dsl_wq");
		 #endif
	if (!lsm6dsl_workqueue)
		return -EINVAL;
#if (CONFIG_ST_HARDWARE_INTERRUPT)
...
#if defined(EMBEDDED_FUNCTION_SIGN_MOTION_SUPPORT)||defined(EMBEDDED_FUNCTION_STEP_COUNTER_SUPPORT)||defined(EMBEDDED_FUNCTION_STEP_DETECTOR_SUPPORT)
...
static int _lsm6dsl_enable_sensors(struct lsm6dsl_sensor_data *sdata)
{
	...
	case LSM6DSL_SIGN_MOTION:
		...
		break;
#endif
#if defined(EMBEDDED_FUNCTION_STEP_COUNTER_SUPPORT)&&defined(EMBEDDED_FUNCTION_STEP_DETECTOR_SUPPORT)
	case LSM6DSL_STEP_COUNTER
	case LSM6DSL_STEP_DETECTOR:
		err = lsm6dsl_enable_pedometer(sdata, true);
		if (err < 0)
...
static int _lsm6dsl_disable_sensors(struct lsm6dsl_sensor_data *sdata)
{
	...
	case LSM6DSL_STEP_DETECTOR:
		...
		break;
#endif
#if defined(EMBEDDED_FUNCTION_TILT_SUPPORT)
	case LSM6DSL_TILT:
		err = lsm6dsl_write_data_with_mask(sdata->cdata,
				LSM6DSL_TILT_EN_ADDR,
				LSM6DSL_TILT_EN_MASK,
...
static int lsm6dsl_init_sensors(struct lsm6dsl_data *cdata)
{
	...
	//1. setting acc power down mode 
	err = lsm6dsl_write_data_with_mask(cdata,LSM6DSL_ACCEL_ODR_ADDR,
					   LSM6DSL_ACCEL_ODR_MASK,0x00,
					   true);
	if (err < 0)
		return err;


	// setting acc full-scale 8G
	err = lsm6dsl_write_data_with_mask(cdata,LSM6DSL_ACCEL_FS_ADDR,
					   LSM6DSL_ACCEL_FS_MASK,LSM6DSL_ACCEL_FS_8G_VAL,
					   true);
	...
	//1. setting gyro power down mode
	err = lsm6dsl_write_data_with_mask(cdata,LSM6DSL_GYRO_ODR_ADDR,
					   LSM6DSL_GYRO_ODR_MASK,0x00,
					   true);
	...
	//2 setting gyro full-scale 2000dps
	err = lsm6dsl_write_data_with_mask(cdata,LSM6DSL_GYRO_FS_ADDR,
					   LSM6DSL_GYRO_FS_MASK,LSM6DSL_GYRO_FS_2000_VAL,
					   true);
	if (err < 0)
		return err;
#endif
...
static ssize_t lsm6dsl_chip_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "lsm6dsl");
}
...
#endif
static DEVICE_ATTR(chip_info, 0440,lsm6dsl_chip_info_show, NULL);
static DEVICE_ATTR(enable, 0664,  get_enable, set_enable);
static DEVICE_ATTR(acc_enable, 0664,  get_enable, set_enable);
static DEVICE_ATTR(gyro_enable, 0664,  get_enable, set_enable);

static DEVICE_ATTR(sampling_freq, 0664, get_sampling_freq,
		   set_sampling_freq);
static DEVICE_ATTR(polling_rate, 0664, get_polling_rate,
		   set_polling_rate);
static DEVICE_ATTR(acc_delay, 0664, get_polling_rate,
		   set_polling_rate);
static DEVICE_ATTR(gyro_delay, 0664, get_polling_rate,set_polling_rate);
...
int lsm6dsl_common_probe(struct lsm6dsl_data *cdata, int irq, u16 bustype)
{
	...
	for (i = 0; i < LSM6DSL_SENSORS_NUMB; i++) {
		...
		if (i == LSM6DSL_ACCEL) {
			...
			sdata->c_gain = lsm6dsl_fs_table[i].fs_avl[2].gain;//acc full-scale is 8G
			sdata->poll_interval = 1000 / sdata->c_odr;
		}
		else if(i == LSM6DSL_GYRO) {
			sdata->c_odr = lsm6dsl_odr_table.odr_avl[0].hz;
			sdata->c_gain = lsm6dsl_fs_table[i].fs_avl[3].gain;//gyro full-scale is 2000dps
			sdata->poll_interval = 1000 / sdata->c_odr;
		}
		#ifdef EMBEDDED_FUNCTION_STEP_COUNTER_SUPPORT
        ...
        kobject_uevent(&sensor_dev->dev.kobj, KOBJ_ADD);
		...
		kobject_uevent(&sdata->input_dev->dev.kobj, KOBJ_ADD);
	}
