...
static const char *const flash_ic_gpio_names[FLASH_IC_GPIO_MAX] = {
    ...
    "flash-torch-en-gpios", /*gpio-76*/
    "flash-en-gpios",       /*gpio-76 connect to strobe*/
    "flash-sync-gpios",     /*gpio-76 connect to tx*/
};
...
static int sprd_flash_ic_cfg_value_torch(void *drvd, uint8_t idx,
  ...
  if (drv_data->i2c_info) {
    flash_ic_driver_reg_write(drv_data->i2c_info, 0x05, 0x3C);
  }
...
static int
sprd_flash_ic_cfg_value_preflash(void *drvd, uint8_t idx,
                                 struct sprd_flash_element *element) {
  struct flash_driver_data *drv_data = (struct flash_driver_data *)drvd;
...
static int
sprd_flash_ic_cfg_value_highlight(void *drvd, uint8_t idx,
                                  struct sprd_flash_element *element) {
  struct flash_driver_data *drv_data = (struct flash_driver_data *)drvd;
...
static int sprd_flash_ic_driver_probe(struct i2c_client *client,
  ...
  ret = of_property_read_u32(dev->of_node, "sprd,torch-level",
                             &pdata->flash_cfg.torch_level);
  if (ret)
    pr_info("torch-level no cfg\n");

  ret = of_property_read_u32(dev->of_node, "sprd,preflash-level",
                             &pdata->flash_cfg.preflash_level);
  if (ret)
    pr_info("preflash-level no cfg\n");

  ret = of_property_read_u32(dev->of_node, "sprd,highlight-level",
                             &pdata->flash_cfg.highlight_level);
  if (ret)
    pr_info("highlight-level no cfg\n");
  ret = of_property_read_u32(dev->of_node, "sprd,lvfm-enable",
                             &pdata->flash_cfg.lvfm_enable);
  if (ret)
