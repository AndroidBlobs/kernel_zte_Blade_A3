...
static bool __found_offset(struct f2fs_sb_info *sbi, block_t blkaddr, pgoff_t dirty, pgoff_t pgofs,
							int whence)
{
	switch (whence) {
	case SEEK_DATA:
...
static loff_t f2fs_seek_block(struct file *file, loff_t offset, int whence)
{
	...
	for (; data_ofs < isize; data_ofs = (loff_t)pgofs << PAGE_SHIFT) {
		...
		for (; dn.ofs_in_node < end_offset;
			...
			if (__found_offset(F2FS_I_SB(inode), blkaddr, dirty, pgofs, whence)) {
				f2fs_put_dnode(&dn);
				goto found;
			}
		}
